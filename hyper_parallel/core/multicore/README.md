...
- [MegaMoe 动态 token 设计与验证](docs/mega_moe_dynamic_tokens_design.md)
...
- EP 必须覆盖整个默认 world，专家数 `E` 可被 EP 整除。`H/I/E/K/EP` 在构造后固定。
  `local_num_tokens=T` 保留固定长度语义，要求 T 为正的 128 倍数；
  `max_local_num_tokens=T_cap` 允许每次、每 rank 的实际 T 在 `[0,T_cap]` 内变化。
  两个参数必须恰好提供一个；芯片及后端支持范围不变。
...
将上例的 `local_num_tokens=1024` 改为 `max_local_num_tokens=4096`，即可连续传入
224、240、1、4096 或 0 行的真实输入。各 rank 不必等长，也不要求实际 T 是 128 的倍数。
零 token rank 仍必须调用前后向；它可能接收其他 rank 的专家计算任务。

内部复用 counts all-gather，同时交换实际长度和梯度参与策略，按组内最大 T 向上对齐到 128
选择任务图。输入、路由和输出不补位；缓存最多保留 4 个计划，尚待反向的图另行持有自己的计划。
源端 SHMEM 始终按对齐后的 T_cap 预留，切换长度不重新分配 SHMEM。

同组配置、调用顺序和 grad-enabled/hidden/专家参数的 `requires_grad` 策略必须一致。
支持全组一致地冻结输入或专家；不支持单 rank 跳过层或独立改变反向参与策略。
上限或接收容量溢出时全组在启动 MegaKernel 前报错；不会自动截断或扩容。
该接口面向 eager 执行，暂不支持符号动态 shape 编译或 graph capture。
...
源端 permute/combine 输出为本次实际 `T * K` 行；动态模式的对称 source 区按上限预留。每次 forward 保存独立的接收数据和容量，支持后续路由变化。
...
动态模式的上述容量公式使用向上对齐到 128 的 `T_cap` 代替 T。
...
已验证非重入 checkpoint、跨 stream 串行提交和多个未反向前向；`retain_graph=True` 暂未验证。
所有 backward 完成后，各 rank 按相同顺序调用每层的幂等 `close()`，并在销毁进程组前完成关闭。
最后一个共享 owner 在有活动 lease 或待反向图时拒绝关闭；调用方仍须保证各 rank 同步关闭。
