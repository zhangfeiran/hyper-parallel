...
at::Tensor allocate_token_gradient(
    const at::Tensor& grad, const at::Tensor& sorted_indices, int64_t num_tokens, int64_t top_k) {
    ...
    TORCH_CHECK(grad.scalar_type() == at::kBFloat16 || grad.scalar_type() == at::kHalf ||
                grad.scalar_type() == at::kFloat, "Gradient must be BF16, FP16 or FP32.");
    return at::empty({num_tokens, grad.size(1)}, grad.options());
}

at::Tensor moe_token_permute_grad_npu(
    const at::Tensor& grad, const at::Tensor& sorted_indices, int64_t num_tokens, int64_t top_k) {
...
TORCH_LIBRARY_IMPL(hyper_parallel, PrivateUse1, m) {
    m.impl("moe_token_permute_grad", &moe_token_permute_grad_npu);
}

TORCH_LIBRARY_IMPL(hyper_parallel, Meta, m) {
    m.impl("moe_token_permute_grad", &allocate_token_gradient);
}
